...
void solve(){
	...
	while(cur>s){
		...
		if(c1.empty()&&!c2.empty()){
			...
			auto cost2 = c2.top();
			c2.pop();
			ll rem2 = cost2.s*1LL*(cost2.f-(cost2.f/2LL));
			ll rem1 = cost1.s*1LL*(cost1.f-(cost1.f/2LL));
			// cout<<rem1<<endl;
			if(cur-rem1<=s){
				ans++;
				cur-=rem1;
				...
				if(!c1.empty()){
					cost3 = c1.top();
					c1.pop();
					rem3 = cost3.s*1LL*(cost3.f-(cost3.f/2LL));
				}
				ll temp = cost1.f/2LL;
				ll rem4 = cost1.s*1LL*((temp)-(temp/2LL));
				...
				if(rem1+rem5>=rem2){
					...
					if(rem5==rem3){
						c1.push({cost1.f/2LL,cost1.s});
						c1.push({cost3.f/2LL,cost3.s});
					}
					else{ 
						c1.push({(temp/2LL),cost1.s});
						if(rem3!=-1) c1.push(cost3);
					}
					c2.push(cost2);
					cur-=(rem1+rem5);
					lastOne = rem5;
	...
	if(lastOne!=-1&&cur+lastOne<=s) ans--;
	// cout<<cur<<endl;
	cout<<ans<<endl;
