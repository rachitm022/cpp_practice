...
vector<vi> dist(301,vi(301,INT_MAX));
vector<vi> nex(301,vi(301,-1));
void solve(){
	int n;
	cin>>n;
	vector<int> adj[n+1];
	...
	vi temp;
	fl(i,0,cntL+1){
		temp = path(leaves[i],leaves[i+1]);
		// pr(temp,sz(temp));
		auto it = temp.begin();
		++it;
		ans.insert(ans.end(),it,temp.end());
	}
	// pr(ans,sz(ans));
	if(ans.size()>2*n-1){
		cout<<-1<<endl;return;
